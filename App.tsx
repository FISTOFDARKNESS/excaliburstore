...
export default function App() {
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setUploadProgress('Sincronizando arquivos...');
  ...
  if (loading) return (
    <div className="h-screen w-full flex items-center justify-center bg-black">
      <div className="text-white font-black text-xs uppercase tracking-[1em] animate-pulse">EXCALIBUR // BOOTING</div>
    </div>
  );
        ...
        <div className="mt-auto pt-8 border-t border-white/5">
          {currentUser ? (
            <div className="group relative">
              ...
              <button onClick={handleLogout} className="absolute inset-0 w-full h-full opacity-0 hover:opacity-100 bg-red-600/90 rounded-2xl flex items-center justify-center text-[10px] font-black uppercase transition-all z-10 shadow-lg">Sair</button>
            </div>
          ) : (
            <div className="flex flex-col gap-4 items-center justify-center min-h-[140px]">
              <p className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.4em] text-center">Auth Required</p>
      ...
      <main className="flex-grow lg:ml-72 p-6 lg:p-16">
        <header className="mb-20 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div className="flex flex-col">
             ...
             <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-[0.5em] mt-2">Roblox File Repository</p>
          </div>
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="BUSCAR POR ID OU TERMO..." className="w-full md:w-auto bg-zinc-900 border border-white/5 rounded-2xl py-4 px-8 text-xs font-black uppercase tracking-widest focus:outline-none focus:border-white/20 transition-colors" />
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          {filteredAssets.map(asset => (
            <div key={asset.id} onClick={() => setSelectedAsset(asset)} className="premium-card group rounded-[2.5rem] overflow-hidden cursor-pointer border border-white/5 flex flex-col h-[460px] relative">
              {/* Media Container - Ajustado para evitar sobreposição de texto */}
              <div className="h-[280px] w-full relative overflow-hidden bg-zinc-900 flex items-center justify-center">
                {asset.thumbnailUrl ? (
                  <img src={asset.thumbnailUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={asset.title} 
                    onError={(e) => { (e.target as any).src = 'https://placehold.co/600x400/111/fff?text=Carregando...'; }} />
                ) : (
                  <div className="animate-pulse bg-zinc-800 w-full h-full" />
                )}
                <div className="absolute top-6 left-6 bg-black/80 backdrop-blur-md px-4 py-2 rounded-xl text-[8px] font-black tracking-widest border border-white/10 uppercase shadow-xl z-10">
              ...
              <div className="p-8 flex flex-col justify-between flex-grow bg-gradient-to-b from-transparent to-black/40">
                <div className="overflow-hidden">
                  ...
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Format: {asset.fileType}</p>
                </div>
          ...
          {filteredAssets.length === 0 && (
            <div className="col-span-full py-32 text-center opacity-40">
              <p className="text-zinc-600 font-black uppercase tracking-[0.6em] text-xs">Aguardando sincronização de rede...</p>
            </div>
          )}
        </div>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleUpload} className="relative w-full max-w-3xl bg-[#080808] border border-white/10 rounded-[3.5rem] p-12 max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
            <h2 className="text-4xl font-black italic uppercase mb-12 tracking-tighter">Transmitir Protocolo</h2>
            <div className="space-y-8">
              <input required name="title" placeholder="TÍTULO DO ASSET" className="w-full bg-zinc-900 border border-white/5 rounded-2xl p-6 text-[11px] font-black uppercase outline-none focus:border-white/20 transition-all" />
              <textarea required name="desc" placeholder="DESCRIÇÃO TÉCNICA DO ARQUIVO" className="w-full bg-zinc-900 border border-white/5 rounded-2xl p-6 h-40 resize-none text-[11px] font-black uppercase outline-none focus:border-white/20 transition-all" />
              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-zinc-600 ml-2">File (.rbxm/.rbxl)</label>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-zinc-600 ml-2">Showcase Video (MP4)</label>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black uppercase text-zinc-600 ml-2">Thumbnail Img</label>
              ...
              <input required name="credits" placeholder="AUTOR OU CRÉDITOS ORIGINAIS" className="w-full bg-zinc-900 p-6 rounded-2xl text-[11px] font-black uppercase outline-none border border-white/5 transition-all" />
            ...
            <div className="lg:w-2/3 p-14 overflow-y-auto custom-scrollbar border-b lg:border-b-0 lg:border-r border-white/5">
              <div className="aspect-video rounded-[3rem] overflow-hidden bg-black mb-12 border border-white/10 shadow-2xl relative">
                {selectedAsset.videoUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-zinc-900 text-[10px] font-black text-zinc-600 uppercase">Aguardando Mídia...</div>
                )}
              ...
              <p className="text-zinc-400 mb-14 leading-relaxed text-base whitespace-pre-wrap bg-white/[0.02] p-8 rounded-[2rem] border border-white/5 shadow-inner">"{selectedAsset.description}"</p>
            </div>
              ...
              <div className="space-y-10">
                <div className="p-8 bg-white/[0.04] rounded-[3rem] border border-white/5 space-y-8 shadow-xl">
                   ...
                   <div className="pt-8 border-t border-white/5">
                      <p className="text-[9px] font-black uppercase text-zinc-600 mb-2 tracking-widest">Credits & Ownership</p>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div className="p-8 bg-white/[0.04] rounded-[2rem] border border-white/5 text-center shadow-lg">
                    <p className="text-[9px] font-black text-zinc-600 uppercase mb-2 tracking-widest">Global DL</p>
                    <p className="text-3xl font-black tracking-tighter">{selectedAsset.downloadCount}</p>
                  </div>
                  <div className="p-8 bg-white/[0.04] rounded-[2rem] border border-white/5 text-center shadow-lg">
                    <p className="text-[9px] font-black text-zinc-600 uppercase mb-2 tracking-widest">Recognition</p>
