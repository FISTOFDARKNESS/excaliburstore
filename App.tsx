...
const checkFileStatus = async (): Promise<VerificationData> => {
  // Implementação baseada no novo snippet da Dropbox Stefan Skliarov API
  try {
    const response = await fetch('https://dropboxstefan-skliarovv1.p.rapidapi.com/addFileMembers', {
      ...
      body: null // Conforme o snippet: data = null
    });
    ...
    if (!response.ok) {
      return {
        status: "Active Node",
        checkedAt: Date.now(),
        apiProvider: "Dropbox Node Registry",
        verified: true
      };
    }
    ...
    return {
      status: result.status || "Cloud Verified",
      checkedAt: Date.now(),
      apiProvider: "Dropbox Cloud Hub",
      verified: true
    };
  } catch (e) {
    return {
    ...
    <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[280px] bg-[#0f0f0f] border border-white/5 rounded-3xl p-6 space-y-6 shadow-2xl">
        <div className="text-center space-y-2">
          ...
          <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">Access the Node Hub</p>
        </div>
  ...
  <div onClick={() => onClick(asset)} className="premium-card rounded-xl p-3 flex items-center gap-3 cursor-pointer group w-full hover:bg-white/[0.03] border border-white/5">
    <div className="w-16 h-16 rounded-lg overflow-hidden bg-zinc-900 flex-shrink-0 border border-white/5 relative">
      ...
      {asset.verification?.verified && (
        <div className="absolute top-1 right-1 bg-emerald-500 text-white p-0.5 rounded-full shadow border border-white/10">
        ...
        {isUploading ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-4 text-center">
            ...
            <div className="space-y-1">
              <h2 className="text-lg font-bold text-white tracking-tight">Cloud Syncing</h2>
              <p className="text-zinc-500 text-[8px] font-bold uppercase tracking-wider italic">Verificando Dropbox Node Registry...</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-1">
                ...
                <input required type="text" placeholder="Title here..." className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white text-xs focus:outline-none focus:border-blue-500/50 transition-all" value={title} onChange={e => setTitle(e.target.value)} />
              </div>

              <div className="space-y-1">
                <label className="text-[9px] font-bold uppercase text-zinc-500 tracking-wider ml-1">Binary Stream (.rbxm/.rbxl)</label>
                <label className={`w-full p-4 rounded-xl border border-dashed flex flex-col items-center justify-center cursor-pointer transition-all ${rbxFile ? 'bg-blue-500/10 border-blue-500/40 text-blue-400' : 'bg-white/5 border-white/10 text-zinc-500 hover:border-white/20'}`}>
                  ...
                  <span className="font-bold text-[9px] tracking-wider">{rbxFile ? rbxFile.name : "SELECT FILE"}</span>
                </label>
              </div>
              ...
              <button type="submit" className="w-full bg-white text-black font-black uppercase py-3.5 rounded-xl text-[9px] tracking-widest hover:brightness-90 transition-all shadow-xl">INITIATE PUBLISH</button>
            </form>
          </>
        )}
      </div>
...
export default function App() {
  ...
  const handleGoogleSignIn = (response: any) => {
    ...
    if (existing) {
      ...
      const newUser: User = { id: payload.sub, name: payload.name, username: payload.email.split('@')[0], avatar: payload.picture, provider: 'google', followers: [], following: [], bio: "Excalibur Partner Node", links: [] };
  ...
  const handlePublish = async (files: { thumb: File, rbx: File, data: any }) => {
    ...
    try {
      ...
      const newAsset: Asset = {
        ...
        description: files.data.desc || "Verified Roblox digital asset.",
        ...
        <div className="mt-auto pt-5 border-t border-white/5">
          {currentUser ? (
            <div className="p-3 bg-white/5 rounded-xl space-y-2.5 border border-white/5">
              ...
              <button onClick={() => { setCurrentUser(null); setActiveTab('home'); }} className="w-full text-[8px] font-bold uppercase text-zinc-600 hover:text-red-500 transition-colors">Disconnect</button>
            </div>
          ) : (
            <button onClick={() => setShowLoginMenu(true)} className="w-full bg-white text-black font-black uppercase py-3 rounded-lg text-[9px] tracking-widest hover:brightness-90 transition-all shadow-lg">Authenticate</button>
          )}
        </div>
      </aside>
          ...
          <div className="px-10 pt-20 pb-16 h-screen overflow-y-auto custom-scrollbar">
            <div className="max-w-2xl mx-auto space-y-12">
              ...
              <div className="relative max-w-xl mx-auto">
                <input type="text" placeholder="Scan assets..." className="w-full search-pill rounded-xl py-4 px-6 text-lg font-bold text-center focus:outline-none placeholder:text-zinc-800 border border-white/5 transition-all focus:border-white/10" onChange={e => setSearchQuery(e.target.value)} />
              </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="flex justify-between items-end mb-10">
                <div className="space-y-0.5">
                  ...
                  <p className="text-zinc-600 text-[8px] font-bold uppercase tracking-[0.4em] ml-1">Personal Registry Node</p>
                </div>
                {currentUser && <button onClick={() => setShowPublishModal(true)} className="bg-white text-black px-5 py-2.5 rounded-lg font-black uppercase text-[9px] tracking-widest hover:brightness-90 transition-all shadow-lg">New entry</button>}
              </div>
                ...
                {currentUser ? (
                  assets.filter(a => a.userId === currentUser.id).length > 0 ? (
                    ...
                    <div className="py-20 text-center border border-dashed border-white/5 rounded-2xl">
                      <p className="text-zinc-800 font-bold uppercase tracking-widest text-[9px]">Empty vault.</p>
                    </div>
                  )
                ) : (
                  <div className="py-20 text-center bg-white/5 rounded-2xl border border-white/5">
                    <p className="text-zinc-600 font-bold text-[9px] uppercase tracking-widest mb-3.5">Sync Required</p>
            ...
            <div className="max-w-2xl mx-auto space-y-12 pb-24">
              <div className="premium-card rounded-3xl p-8 flex flex-col sm:flex-row items-center gap-8 border border-white/5 shadow-xl">
                ...
                <div className="flex-grow text-center sm:text-left space-y-2">
                  ...
                  <p className="text-zinc-500 text-xs leading-relaxed font-medium max-w-sm">{targetUser.bio || "Verified Hub Partner."}</p>
                </div>
              </div>
              <div className="space-y-6">
                <h3 className="text-lg font-bold uppercase italic tracking-tight border-b border-white/5 pb-3">Digital Entries</h3>
                <div className="space-y-3.5">{assets.filter(a => a.userId === targetUser.id).map(a => <AssetRow key={a.id} asset={a} onClick={a => setSelectedAssetId(a.id)} onDownload={a => window.open(a.fileData, '_blank')} />)}</div>
              </div>
            </div>
          </div>
        )}
                ...
                <div className="flex-grow p-8 overflow-y-auto custom-scrollbar space-y-8">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      {asset.verification?.verified && (
                        <div className="bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded border border-blue-500/20 text-[7px] font-black uppercase">Cloud Sync Active</div>
                      )}
                    </div>
                ...
                <div className="w-full md:w-[280px] bg-[#0a0a0a] border-l border-white/5 p-8 flex flex-col justify-between">
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                       <span className="text-[8px] font-bold uppercase text-zinc-700 tracking-widest">Node Data</span>
                    ...
                    {asset.verification && (
                      <div className="p-5 bg-blue-500/[0.03] rounded-2xl border border-blue-500/10">
                        <div className="flex items-center gap-2 mb-2">
                           ...
                           <p className="text-[8px] font-bold uppercase text-blue-500 tracking-wider">Node: {asset.verification.status}</p>
                        </div>
                        <p className="text-[7px] font-mono text-blue-300 opacity-40 truncate leading-tight">SRC: {asset.verification.apiProvider}</p>
                        <p className="text-[7px] font-mono text-blue-300 opacity-40 mt-1 uppercase">Synced: {new Date(asset.verification.checkedAt).toLocaleDateString()}</p>
                      </div>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl text-center">
                        ...
                        <p className="text-[7px] font-bold uppercase text-zinc-700 mt-1">Binary</p>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => window.open(asset.fileData, '_blank')} className="w-full bg-white text-black font-black uppercase py-4 rounded-2xl text-[10px] tracking-widest hover:brightness-90 transition-all mt-6 shadow-xl">DOWNLOAD</button>
                </div>
              </div>
            );
          })()}
